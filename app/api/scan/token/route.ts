...
import { runTokenScan, ScanServiceError } from "@/lib/scan-service";
...
async function handler(req: NextRequest) {
  ...
  const rl = checkRateLimit(`token:${ip}`);
  if (!rl.allowed) {
  ...
  try {
    const result = await runTokenScan(validated.address);
...
/** Paid A2MCP endpoint: $0.01 USDT0 via x402 when DEMO_MODE is off. */
export const POST = protectWithX402(handler, "token");
